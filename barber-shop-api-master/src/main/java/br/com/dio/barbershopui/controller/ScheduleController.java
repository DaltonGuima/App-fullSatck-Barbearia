...
public class ScheduleController {
    ...
    // Simplificado o método listMonth usando atStartOfDay() para definir o início
    // do mês.

    @GetMapping("{year}/{month}")
    public ScheduleAppointmentMonthResponse listMonth(@PathVariable final int year, @PathVariable final int month) {
        var yearMonth = YearMonth.of(year, month);
        var startAt = yearMonth.atDay(1).atStartOfDay().atOffset(UTC);
